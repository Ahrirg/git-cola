...
def stop(context, _view: Any) -> None:
    """All done, cleanup"""
    context.view.stop()
...
def new_window(context, filename: str) -> MainWindow:
...
class MainWindow(standard.MainWindow):
    ...
    def __init__(self, context, parent: Any | None = None) -> None:
...
class RebaseTreeWidget(standard.DraggableTreeWidget):
    ...
    def __init__(self, context, comment_char: str, parent: QWidget) -> None:
    ...
    def contextMenuEvent(self, event: QDropEvent) -> None:
        items = self.selected_items()
