...
def xdg_data_home(*args) -> str | UStr:
...
def xdg_data_dirs() -> list[str | UStr]:
    ...
    xdg_data_dirs_env: str | UStr | None = core.getenv('XDG_DATA_DIRS', '')
    if not xdg_data_dirs_env:
        xdg_data_dirs_env: str | UStr | None = '/usr/local/share:/usr/share'
    paths.extend(path for path in xdg_data_dirs_env.split(':') if os.path.isdir(path))
    return paths


def find_first(
    subpath: str, paths: list[str | UStr], validate: Callable = os.path.isfile
) -> str | None:
