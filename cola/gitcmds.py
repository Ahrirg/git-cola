...
def apply_diff(context, filename: str) -> Any:
...
def get_default_remote(context) -> Any:
...
def upstream_remote(context, branch: str | None = None) -> Any:
...
def remote_url(context, remote: str, push: bool = False) -> Any:
...
def diff_index_filenames(context, ref: Any) -> Any:
    """
...
def diff_filenames(context, *args) -> list[Any | str]:
...
def changed_files(context, oid: str) -> list[str]:
...
def diff(context, args: list[str]) -> list[str]:
...
def _parse_diff_filenames(out: UStr) -> list[Any | str]:
    if out:
...
def all_files(context, *args) -> list[str]:
...
def current_branch(context) -> str | UStr:
...
def _read_git_head(context, head, default: str = 'main') -> str:
...
def branch_list(context, remote: bool = False) -> list[Any]:
    """
...
def for_each_ref_basename(context, refs) -> list[Any]:
...
def tracked_branch(context, branch: str | None = None) -> str | None:
...
def log(context, *args, **kwargs) -> str:
...
def commit_diff(context, oid: str) -> str:
...
def oid_diff(context, oid: str, filename: str | None = None) -> Any:
    """Return the diff for an oid"""
    return oid_diff_range(context, oid + '~', oid, filename=filename)


def oid_diff_range(context, start: str, end: str, filename: str | None = None) -> str:
...
def diff_info(context, oid: str, filename: str | None = None) -> str:
    """Return the diff for the specified oid"""
    return diff_range(context, oid + '~', oid, filename=filename)


def diff_range(context, start: str, end: str, filename: str | None = None) -> str:
...
def format_patchsets(
    context, to_export: Any, revs: Any, output: str = 'patches'
) -> tuple[int, str, str]:
    """
...
def export_patchset(
    context, start: str, end: str, output: str = 'patches', **kwargs
) -> Any:
...
def untrack_paths(context, args: list[str]) -> tuple[int, str, str]:
    if not args:
...
def diff_upstream(context, head: str) -> list[Any | str]:
...
def ls_tree(context, path, ref: str = 'HEAD') -> list[tuple[str, str]]:
...
def ls_tree_paths(context, ref: str, *args) -> list[str]:
...
def log_helper(
    context, all: bool = False, extra_args=None
...
def rev_list_range(context, start, end) -> list[tuple[str, str]]:
...
def read_merge_commit_message(context, path) -> str | UStr:
...
def cherry_pick(context, revs) -> tuple[int, str, str] | list:
...
def abort_apply_patch(context) -> tuple[int, str, str]:
...
def abort_cherry_pick(context) -> tuple[int, str, str]:
    """Abort a cherry-pick."""
    # Reset the worktree
...
def abort_merge(context) -> tuple[int, str, str]:
    """Abort a merge"""
    # Reset the worktree
    if prefs.verbose_simple_commands(context):
...
def parse_refs(context, argv) -> list[str]:
...
def prev_commitmsg(context, *args) -> str:
...
def rev_parse(context, name: str) -> str:
...
def cat_file_blob(context, filename: str, oid: str) -> str:
...
def cat_file_from_ref(context, ref: str, filename: str) -> str:
...
def write_blob_path(context, head: str, oid: str, filename: str) -> str:
...
def annex_path(context, head: str, filename: str):
...
def is_binary(context, filename: str) -> bool:
...
def is_valid_ref(context, ref: str) -> bool:
