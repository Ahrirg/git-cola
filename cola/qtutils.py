...
from qtpy.QtCore import QCoreApplication, QEvent, QMimeData, Qt
...
def connect_action_bool(action: QAction, func: Callable) -> None:
    """Connect a triggered(bool) action to a function"""
    action.triggered[bool].connect(func, type=Qt.QueuedConnection)


def connect_button(button: QAbstractButton, func: Callable) -> None:
...
def get(widget: QWidget, default=None):
...
def box(cls, margin: int, spacing: int, *items: Any) -> QBoxLayout:
...
def form(margin: int, spacing: int, *widgets: QWidget) -> QFormLayout:
...
def grid(margin: int, spacing: int, *widgets: QWidget) -> QGridLayout:
...
def splitter(orientation: Qt.Horizontal | Qt.Vertical, *widgets: QWidget):
...
class ComboBox(QtWidgets.QComboBox):
    ...
    def add_item(self, text: str, data: Any | None = None) -> None:
        if data is None:
            data = text
        self.addItem(text)
...
def textbrowser(text: str | None = None) -> QTextBrowser:
...
def link(url: str, text: str, palette: QPalette | None = None) -> str:
    if palette is None:
...
def add_completer(widget: QWidget, items: list[Any]) -> None:
...
def prompt(
    msg: str, title: str | None = None, text: str = '', parent: QWidget | None = None
) -> tuple[str, bool | None]:
...
def paths_from_items(
    items: Any, item_type=TreeWidgetItem.TYPE, item_filter: Any | None = None
):
...
def tree_selection(tree_item: QTreeWidgetItem, items: list[Any]) -> list[Any]:
...
def tree_selection_items(tree_item: QTreeWidgetItem):
...
def selected_item(list_widget: QListWidget, items: Any):
...
def selected_items(list_widget: QListWidget, items: Any):
    """Returns an array of model items that correspond to the selected
    QListWidget rows."""
...
def add_action_bool(
    widget: QWidget, text: str, func: Callable, checked: bool, *shortcuts: Any
) -> _QAction:
    tip = text
...
def add_action(
    widget: QWidget, text: str | None, func: Callable, *shortcuts
) -> _QAction:
...
def add_action_with_icon(
    widget: QWidget, icon: QIcon, text: str | None, func: Callable, *shortcuts: Any
) -> _QAction:
...
def add_action_with_tooltip(
    widget: QWidget, text: str | None, tip: str | None, func: Callable, *shortcuts: Any
) -> _QAction:
    """Create an action with a tooltip"""
    return _add_action(widget, text, tip, func, connect_action, *shortcuts)


def menu_separator(widget: QWidget, text: str = ''):
...
def set_selected_item(widget: QWidget, idx: int) -> None:
...
def add_items(widget: QWidget, items: list[Any]) -> None:
...
def set_items(widget: QWidget, items: list[Any]) -> None:
...
def add_close_action(widget: QWidget) -> _QAction:
...
def default_size(
    parent: QWidget, width: int, height: int, use_parent_height: bool = True
) -> tuple[int, int]:
...
def default_monospace_font() -> QFont:
    if utils.is_darwin():
        family = 'Monaco'
    elif utils.is_win32():
        family = 'Courier'
...
def diff_font(context) -> QFont:
    return font_from_string(diff_font_str(context))


def font_from_string(string) -> QFont:
    qfont = QtGui.QFont()
...
def create_action_button(
    tooltip: str | None = None, icon: QIcon | None = None, visible: bool | None = None
) -> QToolButton:
...
def ok_button(
    text: str, default: bool = True, enabled: bool = True, icon: QIcon | None = None
) -> QPushButton:
    if icon is None:
        icon = icons.ok()
...
def close_button(text: str | None = None, icon: QIcon | None = None) -> QPushButton:
    text = text or N_('Close')
    icon = icons.mkicon(icon, icons.close)
    return create_button(text=text, icon=icon)


def edit_button(enabled: bool = True, default: bool = False) -> QPushButton:
    return create_button(
        text=N_('Edit'), icon=icons.edit(), enabled=enabled, default=default
    )


def refresh_button(enabled: bool = True, default: bool = False) -> QPushButton:
    return create_button(
        text=N_('Refresh'), icon=icons.sync(), enabled=enabled, default=default
    )


def checkbox(text: str = '', tooltip: str = '', checked=None) -> QCheckBox:
    """Create a checkbox"""
    return _checkbox(QtWidgets.QCheckBox, text, tooltip, checked)


def radio(text: str = '', tooltip: str = '', checked=None) -> QRadioButton:
    """Create a radio button"""
    return _checkbox(QtWidgets.QRadioButton, text, tooltip, checked)


def _checkbox(cls, text: str, tooltip: str, checked: bool) -> QWidget:
...
def hide_dock(widget: QWidget) -> None:
...
def rgb(red: int, green: int, blue: int) -> QColor:
...
def rgba(red: int, green: int, blue: int, alpha: int = 255) -> QColor:
...
def hsl(hue: float, saturation: float, lightness: float) -> QColor:
...
def set_scrollbar_values(
    widget: QWidget, hscroll_value: Any, vscroll_value: Any
) -> None:
...
def get_scrollbar_values(widget: QWidget) -> tuple[Any, Any]:
...
def scroll_to_item(widget: QWidget, item: Any) -> None:
...
def select_item(widget: QWidget, item: Any) -> None:
...
def get_selected_values(widget: QWidget, top_level_idx: int, values: Any) -> list[Any]:
...
def get_selected_items(widget: QWidget, idx: int) -> list[Any]:
...
def text_size(font: QFont, text: str) -> tuple[int, int]:
