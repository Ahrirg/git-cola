...
class Interaction:
    ...
    @staticmethod
    def format_command_status(cmd: list[str | UStr] | str, status: int) -> str:
...
%(message)s%(informative_text)s%(details)s\n"""
    ...
    @classmethod
    def run_command(cls, title: str, cmd: list[str]) -> tuple[int, UStr, UStr]:
        cls.log('# ' + title)
    ...
    @classmethod
    def log_status(
        cls, status: int, out: str | UStr, err: str | UStr | None = None
    ) -> None:
